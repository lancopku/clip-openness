...
from datasets import my_cifar10, my_cifar100, imagenet


@DATASET_REGISTRY.register()
class CIFAR10_CIFAR100_ImageNet(DatasetBase):
    dataset_dir = "cifar10_cifar100_imagenet"

    def __init__(self, cfg):
        root = os.path.abspath(os.path.expanduser(cfg.DATASET.ROOT))
        self.superclass2class = {
            'MYCIFAR10': ['airplane', 'automobile', 'bird', 'cat', 'deer', 'dog', 'frog', 'horse', 'ship', 'truck'],
        ...
        cifar10_dataset = my_cifar10.MYCIFAR10(dup_cfg)
        assert cifar10_dataset.num_classes == len(self.superclass2class['MYCIFAR10'])
        cifar100_dataset = my_cifar100.MYCIFAR100(dup_cfg)
        assert cifar100_dataset.num_classes == len(self.superclass2class['MYCIFAR100'])
        imagenet_dataset = imagenet.ImageNet(dup_cfg)
        assert imagenet_dataset.num_classes == len(self.superclass2class['ImageNet'])

        train = cifar10_dataset.train_x + cifar100_dataset.train_x + imagenet_dataset.train_x
        val = cifar10_dataset.val + cifar100_dataset.val + imagenet_dataset.val
        test = cifar10_dataset.test + cifar100_dataset.test + imagenet_dataset.test
