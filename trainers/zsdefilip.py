...
from declip.defilip import defilip_vitb32
...
_MODELS = {
    "ViT-B/32/DeFILIP": "/home/renshuhuai/.cache/defilip/vitb32_defilip.pth.tar",
    "RN50/DeFILIP": "/home/renshuhuai/.cache/defilip/r50_defilip.pth.tar"  # TODO not yet release
}


@TRAINER_REGISTRY.register()
class ZeroshotDeFILIP(TrainerX):
    def build_model(self):
        ...
        with torch.no_grad():
            text_features = defilip_model.encode_text(prompts)
            text_features /= text_features.norm(dim=-1, keepdim=True)

        self.text_features = text_features
        self.defilip_model = defilip_model

    def model_inference(self, image):
        image_features = self.defilip_model.encode_image(image)
        image_features = image_features / image_features.norm(dim=-1, keepdim=True)
        logit_scale = self.defilip_model.logit_scale.exp()
        logits = logit_scale * image_features @ self.text_features.t()
        return logits

    def load_defilip_to_cpu(self, cfg):
        ...
        model = self.build_defilip_model(state_dict or model.state_dict())

        return model

    def build_defilip_model(self, state_dict: dict):  # TODO ugly, should be loaded from cfg
        ckpt = state_dict
        ...
        if vit:
            ...
            model = defilip_vitb32(**kwargs)
        else:
            model = None  # TODO not release yet
...
@TRAINER_REGISTRY.register()
class ZeroshotDeFILIP2(ZeroshotDeFILIP):
    """Prompt ensembling."""
